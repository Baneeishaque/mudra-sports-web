...
function paginate_init($name) {
    ...
    //Pagination
    $sql = "SELECT COUNT(name) AS coun FROM product where category_id='$name'";
    //echo $sql;
    ...
    if ($count < $capacity) {
        display_all($starting_index, $count, $name);
    } else {

        display_pagination($starting_index, $capacity, $count, 1, $name);
    }
}

function paginate($starting_index, $count, $current_page, $name) {
    //Pagination
    $capacity = 16;
    common_display();
    display_pagination($starting_index, $capacity, $count, $current_page, $name);
}
...
function display_all($starting_index, $count, $name) {
    ...
    $sql = "SELECT id,name,modal,description,status FROM product where category_id='$name'";
    $result = mysqli_query($con, $sql);
...
function display_pagination($starting_index, $capacity, $count, $current_page, $name) {
    ...
    echo '<div class="catalog-grid">
                                <ul class=" product-grid">';
    $sql = "SELECT id,name,modal,description,status FROM product where category_id='$name' LIMIT $starting_index,$capacity";
    //echo $sql;
    $result = mysqli_query($con, $sql);
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        echo '<li class="  animated"   data-animation="bounceInUp" >
    ...
    for ($index = 0; $index < $no_of_pages; $index++) {
        if (($index + 1) == $current_page) {
            ...
            if ($index == 0) {
                echo '<a href="products-by-category.php?name=' . $name . '" class="btn btn-primary"><span>1</span></a></li>';
            } else {
                echo '<a href="products-by-category.php?starting_index=' . $ending_index . '&count=' . $count . '&current_page=' . ($index + 1) . '&name=' . $name . '" class="btn btn-primary"><span>' . ($index + 1) . '</span></a></li>';
                $ending_index = $ending_index + $capacity;
            }
        } else {
            echo '<li>';
            if ($index == 0) {
                echo '<a href="products-by-category.php?name=' . $name . '" class="btn btn-default"><span>1</span></a></li>';
            } else {
                echo '<a href="products-by-category.php?starting_index=' . $ending_index . '&count=' . $count . '&current_page=' . ($index + 1) . '&name=' . $name . '" class="btn btn-default"><span>' . ($index + 1) . '</span></a></li>';
                $ending_index = $ending_index + $capacity;
            }
        }
    }
